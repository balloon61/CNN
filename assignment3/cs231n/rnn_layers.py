...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # Get tha value from cache
    x, prev_h, Wx, Wh, b, next_h = cache
    # calculate the gradient
    # the grad of tanh is (1-x**2)
    dtanh = (1 - next_h**2) * dnext_h # shape (N, H)
    # calculate db
    db = np.sum(dtanh, axis=0) # shape (H,)
    # dWx is x.T * dout, and dx = dout * Wx.T
    dWx = (x.T).dot(dtanh) # shape (D, H)
    dx = dtanh.dot(Wx.T)# shape (N, D)
    # dWh = prev_h.T * dout, dprev_h * Wh.T
    dprev_h = dtanh.dot(Wh.T)# shape (N, H)
    dWh = (prev_h.T).dot(dtanh)# shape (H, H)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    """
    This part implement RNN forward by calling rnn unit which is rnn_step_forward again and again
    save each hidden states and cache
    """
    # Get the shape
    N, T, _ = x.shape
    _, H = h0.shape
    ...
    h = np.zeros((N, T, H))
    # define caches as a dictionary
    caches = {}
    # rnn_step_forward for each hidden state unit
    for i in range(T):
        h0, cache = rnn_step_forward(x[:, i, :], h0, Wx, Wh, b)
        h[:, i, :] = h0
        caches[i] = cache
    # pass
...
def rnn_backward(dh, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    """
    This part implement RNN backward by calling rnn unit which is rnn_step_backward again and again
    calculate dx, dprev_h, dWx, dWh, db
    """
    # since I want to initialize dx, dprev_h, dWx, dWh, db, I need to get N, T, D, H first
    x, _, _, _, _, _ = cache[0]
    # get the shape (size)
    _, T, H = dh.shape
    N, D = x.shape
    # initial dx, dprev_h, dWx, dWh, db
    dx = np.zeros((N, T, D))
    dprev_h = np.zeros((N, H))
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
    db = np.zeros((H, ))
    # backward for each rnn_step_backward unit
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # give word vectors for all input words
    out = W[x, :]
    # save the x, w to cache
    cache = (x, W)
    # pass
...
def word_embedding_backward(dout, cache):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # get the varibale in cache
    x, W = cache
    # get the size of W
    V, D = W.shape
    # initialize the dW
    dW = np.zeros((V, D))
    # use numpy add at to calculate the dW by using x, dout
    np.add.at(dW, x, dout)
    # pass
