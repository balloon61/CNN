...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # (4) Use a (temporal) affine transformation to compute scores over the vocabulary at every timestep using the hidden states   
        ...
        # backward
        # backward (4) Use a (temporal) affine transformation to compute scores over the vocabulary at every timestep using the hidden states   
        dout, dW3, db3 = temporal_affine_backward(dout, cache3)
        
        # backward (3) Use vanilla RNN
        # if self.cell_type == 'rnn':
        dout, dprev_h, dWx, dWh, db = rnn_backward(dout, cache2)
        
        # backward (2) Use a word embedding layer to transform the words in captions_in from indices to vectors
        dW2 = word_embedding_backward(dout, cache1)
        
        # backward (1) Use an affine transformation to compute the initial hidden state from the image features.
        dout, dW1, db1 = affine_backward(dprev_h, cache0)
        
        # save the gradient to dictionary
        grads["W_vocab"] = dW3
        grads["b_vocab"] = db3
        grads["Wx"] = dWx
        grads["Wh"] = dWh
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            # (2) Make an RNN step using the previous hidden state
            # if self.cell_type == "rnn":
