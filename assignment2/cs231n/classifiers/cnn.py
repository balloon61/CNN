...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        pw = 2
        ph = 2
        # define ThreeLayerConvNet parameters 
        self.params['W1'] = np.random.normal(scale = weight_scale, size = (num_filters, input_dim[0], filter_size, filter_size)) 
        self.params['b1'] = np.zeros((num_filters, )) 
        self.params['W2'] = np.random.normal(scale = weight_scale, size = (num_filters * int((input_dim[1] - ph) / ph + 1) * int((input_dim[2] - pw) / pw + 1), hidden_dim)) 
    ...
    def loss(self, X, y=None):
        ...
        # forward follows the architecture conv - relu - 2x2 max pool - affine - relu - affine - softmax
        ...
        # use the last out for the scores 
        scores = out3
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        # calculate loss and grad from softmax
        loss, grad = softmax_loss(scores, y)  
        
        # backward follows the architecture conv - relu - 2x2 max pool - affine - relu - affine - softmax
        ...
        # loss add reg
        loss = loss + self.reg * (1/2) * (np.sum(W1*W1) + np.sum(W2*W2) + np.sum(W3 * W3))
        
        # grad add reg and save it to dictionary
        grads['W3'] = dw3 + self.reg * W3
        grads['W2'] = dw2 + self.reg * W2
        grads['W1'] = dw1 + self.reg * W1
        # save db to dictionary
        grads['b3'] = db3 
        grads['b2'] = db2 
        grads['b1'] = db1 
